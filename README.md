# Ternary_Contour
Plots a series of 3-dimensional data on a ternary diagram in the form of a contour map. Contours are based on 'spatial'-density of datapoints in the domain of the ternary plot, which is a representation of the bivariate (joint)  PDF constructed from the bivariate kernel density estimate. Multiple 'sets' of series of 3D data can be plotted on the same figure, each with different colours of their contour maps.
