...
def Ternary_Contour(dataset, type, NumLevels, ContLines, FigureSavePath, FileName, Colour):
    ...
    ** Colour : In quotations (such as 'Blues') the colour of the contour plot. Options available according to 'cmap' of matplotlib (https://matplotlib.org/stable/tutorials/colors/colormaps.html).
    ...
    def Tern_Base():
        ...
        """
        with plt.style.context(['Ternary_Base']):
        ...
        if type == 'silicate':
            with plt.style.context(['Ternary_Base_StoichRef']):
                ax.plot([0.29, -0.29], [0.4, 0.4], linestyle=':', color='dimgrey')
                ax.annotate('Serp', xy=(0.3, 0.4), xytext = (0.32, 0.39))
                
                ax.plot([0.21, -0.21], [0.57, 0.57], linestyle=':', color='dimgrey')
                ax.annotate('Sap', xy=(0.21, 0.57), xytext = (0.23, 0.56))
                
                ax.plot([0.329, -0.329], [0.333, 0.333], linestyle='--', color='dimgrey')
                ax.annotate('Ol', xy=(0.333, 0.333), xytext = (0.35, 0.32))
                
                ax.plot([0.24, -0.24], [0.5, 0.5], linestyle='--', color='dimgrey')
                ax.annotate('Py', xy=(0.25, 0.50), xytext = (0.27, 0.49))
            
            ax.annotate('Si + Al', xy=(0.2, 1.), xytext = (-0.06, 1.03))
            ax.annotate('Fe', xy=(-0.55,0.0), xytext = (-0.57,-0.03))
            ax.annotate('Mg', xy=(0.55,0.0), xytext = (0.53, -0.03))
        ...
        if type == 'silicateHydration':
            ...
            ax.annotate('Si + Al', xy=(0.01, 1.0), xytext = (-0.06, 1.05))
            ax.annotate('O', xy=(-0.55,0.), xytext = (-0.57,-0.03))
            ax.annotate('Mg + Fe', xy=(0.55,0.0), xytext = (0.53, -0.03))
    ...
    def triplot(dataset, mask, legend, type, dataform):
        ...
        ** type : Either 'silicate' [Si+Al, Mg, Fe],  'silicateHydration' [Si+Al, Mg+Fe, O], or 'sulfide' [S, Ni, Fe].
        ...
        with plt.style.context(['Preferences_Base']):
    #        fig, ax = plt.subplots()
            for i in range(len(dataset)):
                ax.plot(tri_x[i], tri_y[i])

        ax.axis('off')
        with plt.style.context(['Icarus_Base']):
    ...
    with plt.style.context(['Icarus_Base']):
        ...
        xx_Ternary, yy_Ternary = np.mgrid[xmin_Ternary:xmax_Ternary:200j, ymin_Ternary:ymax_Ternary:200j]
